	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            ...
            if (status.indexOf("INACTIVE") > -1) {
				controller += '<form id="formActiveExtractSettingTransaction" action="' + baseUrl + 'ExtractSettingTransaction/Active" method="post"  class="btn btn-info btn-sm fa fa-check" title="Active Extract Setting">' +
                              '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                              '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                              '</form>';
			}
			else {
				controller += '<form id="formInactiveExtractSettingTransaction" action="' + baseUrl + 'ExtractSettingTransaction/Inactive" method="post"  class="btn btn-info btn-sm fa fa-close " title="Inactive Extract Setting">' +
                              '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                              '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                              '</form>';
			}
			controller += '<form id="formDelete" action="' + baseUrl + 'ExtractSettingTransaction/Delete" method="post"  class="btn btn-info btn-sm fa fa-trash-o" title="Delete">' +
                          '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                          '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                          '</form><div>';
