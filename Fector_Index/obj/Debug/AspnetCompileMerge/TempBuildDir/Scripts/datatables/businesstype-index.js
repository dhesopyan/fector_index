$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			var controller = '<div><a href="' + baseUrl + 'BusinessType/Edit/' + id + '/" title="Edit"><span class="btn btn-info btn-sm fa fa-edit"></span><a>' +
                             '<input id="id" name="id" type="hidden" value="' + id + '">' +
                             '</form>';
			if (status.indexOf("INACTIVE") > -1) {
				controller += '<form id="formActiveBusinessType" action="' + baseUrl + 'BusinessType/Active" method="post"  class="btn btn-info btn-sm fa fa-check" title="Active Business Type">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			else {
				controller += '<form id="formInactiveBusinessType" action="' + baseUrl + 'BusinessType/Inactive" method="post"  class="btn btn-info btn-sm fa fa-close " title="Inactive Business Type">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			controller += '<form id="formDelete" action="' + baseUrl + 'BusinessType/Delete" method="post"  class="btn btn-info btn-sm fa fa-trash-o" title="Delete">' +
                          '<input id="id" name="id" type="hidden" value="' + id + '">' +
                          '</form><div>';
	...
	$('#myTable tbody').on("click", "#formInactiveBusinessType", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to inactive this businees type?", function (result) {
			if (result) {
				obj.submit();
			}
		});
	});
	$('#myTable tbody').on("click", "#formActiveBusinessType", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to active this business type?", function (result) {
			if (result) {
				obj.submit();
			}
		});
