$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			if (status.indexOf("INACTIVE") > -1) {
			    controller += '<form id="formActiveUAC" action="' + baseUrl + 'UAC/Active" method="post"  class="btn btn-info btn-sm fa fa-check" title="Active UAC">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			else {
			    controller += '<form id="formInactiveUAC" action="' + baseUrl + 'UAC/Inactive" method="post"  class="btn btn-info btn-sm fa fa-close " title="Inactive UAC">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
	...
	$('#myTable tbody').on("click", "#formInactiveUAC", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to inactive this user level?", function (result) {
			if (result) {
				obj.submit();
			}
		});
	});
	$('#myTable tbody').on("click", "#formActiveUAC", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to active this user level?", function (result) {
			if (result) {
				obj.submit();
			}
		});
