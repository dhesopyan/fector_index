$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			var controller = '<div><a href="' + baseUrl + 'DocumentLHBU/Edit/' + id + '/" title="Edit"><span class="btn btn-info btn-sm fa fa-edit"></span><a>' +
                             '<input id="id" name="id" type="hidden" value="' + id + '">' +
                             '</form>';
			if (status.indexOf("INACTIVE") > -1) {
				controller += '<form id="formActiveDocumentLHBU" action="' + baseUrl + 'DocumentLHBU/Active" method="post"  class="btn btn-info btn-sm fa fa-check" title="Active Document LHBU">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			else {
				controller += '<form id="formInactiveDocumentLHBU" action="' + baseUrl + 'DocumentLHBU/Inactive" method="post"  class="btn btn-info btn-sm fa fa-close " title="Inactive Document LHBU">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			controller += '<form id="formDelete" action="' + baseUrl + 'DocumentLHBU/Delete" method="post"  class="btn btn-info btn-sm fa fa-trash-o" title="Delete">' +
                          '<input id="id" name="id" type="hidden" value="' + id + '">' +
                          '</form><div>';
	...
	$('#myTable tbody').on("click", "#formInactiveDocumentLHBU", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to inactive this Document LHBU?", function (result) {
			if (result) {
				obj.submit();
			}
		});
	});
	$('#myTable tbody').on("click", "#formActiveDocumentLHBU", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to active this Document LHBU?", function (result) {
			if (result) {
				obj.submit();
			}
		});
