$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			var controller = '<div><form id="formApprove" action="' + baseUrl + 'DocumentLHBU/Approve" method="post"  class="btn btn-info btn-sm fa fa-check" title="Approve">' +
                             '<input id="id" name="id" type="hidden" value="' + id + '">' +
                             '</form>';
			controller += '<form id="formReject" action="' + baseUrl + 'DocumentLHBU/Reject" method="post"  class="btn btn-info btn-sm fa fa-remove" title="Reject">' +
                          '<input id="id" name="id" type="hidden" value="' + id + '">' +
                          '</form><div>';
	...
	$('#myTable tbody').on("click", "#formApprove", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to approve this document LHBU?", function (result) {
			if (result) {
				obj.submit();
			}
		});
	...
	$('#myTable tbody').on("click", "#formReject", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to reject this document LHBU?", function (result) {
			if (result) {
				obj.submit();
			}
		});
