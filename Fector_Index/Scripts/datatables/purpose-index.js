$(document).ready(function () {
    ...
    var table = $('#myTable').DataTable({
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            ...
            var controller = '<div><a href="' + baseUrl + 'Purpose/Edit/' + id + '/" title="Edit"><span class="btn btn-info btn-sm fa fa-edit"></span><a>';
            if (status.indexOf("INACTIVE")>-1)
            {
                controller += '<form id="formActive" action="' + baseUrl + 'Purpose/Active" method="post"  class="btn btn-info btn-sm fa fa-check" title="Active Data">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
            }
            else
            {
                controller += '<form id="formInactive" action="' + baseUrl + 'Purpose/Inactive" method="post"  class="btn btn-info btn-sm fa fa-close " title="Inactive Data">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
            }
            controller += '<form id="formDelete" action="' + baseUrl + 'Purpose/Delete" method="post"  class="btn btn-info btn-sm fa fa-trash-o" title="Delete">' +
                          '<input id="id" name="id" type="hidden" value="' + id + '">' +
                          '</form><div>';
