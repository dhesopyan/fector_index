$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			if (branchid == '888' || status == 'REJECTED') {
			    controller += '<div><a href="' + baseUrl + 'ExchangeTransaction/Edit?id=' + id + '" title="Edit"><span class="btn btn-info btn-sm fa fa-edit"></span><a>' +
                                '<input id="id" name="id" type="hidden" value="' + id + '">' +
                                '</form>';
			}
			controller += '<a href="' + baseUrl + 'ExchangeTransaction/Viewed?id=' + id + '" title="Viewed"><span class="btn btn-info btn-sm fa fa-eye"></span><a>';
