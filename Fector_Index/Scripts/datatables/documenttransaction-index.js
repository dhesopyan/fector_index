$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			var controller = '<div><a href="' + baseUrl + 'DocumentTransaction/Edit/' + id + '/" title="Edit"><span class="btn btn-info btn-sm fa fa-edit"></span><a>' +
                             '<input id="id" name="id" type="hidden" value="' + id + '">' +
                             '</form>';
			if (status.indexOf("INACTIVE") > -1) {
				controller += '<form id="formActiveDocumentTransaction" action="' + baseUrl + 'DocumentTransaction/Active" method="post"  class="btn btn-info btn-sm fa fa-check" title="Active Document Transaction">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			else {
				controller += '<form id="formInactiveDocumentTransaction" action="' + baseUrl + 'DocumentTransaction/Inactive" method="post"  class="btn btn-info btn-sm fa fa-close " title="Active Document Transaction">' +
                              '<input id="id" name="id" type="hidden" value="' + id + '">' +
                              '</form>';
			}
			controller += '<form id="formDelete" action="' + baseUrl + 'DocumentTransaction/Delete" method="post"  class="btn btn-info btn-sm fa fa-trash-o" title="Delete">' +
                          '<input id="id" name="id" type="hidden" value="' + id + '">' +
                          '</form><div>';
	...
	$('#myTable tbody').on("click", "#formInactiveDocumentTransaction", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to inactive this document transaction?", function (result) {
			if (result) {
				obj.submit();
			}
		});
	});
	$('#myTable tbody').on("click", "#formActiveDocumentTransaction", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to active this document transaction?", function (result) {
			if (result) {
				obj.submit();
			}
		});
