$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
			...
			var controller = '<div><form id="formApprove" action="' + baseUrl + 'ExtractSettingTransaction/Approve" method="post"  class="btn btn-info btn-sm fa fa-check" title="Approve">' +
                             '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                             '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                             '</form>';
			controller += '<form id="formReject" action="' + baseUrl + 'ExtractSettingTransaction/Reject" method="post"  class="btn btn-info btn-sm fa fa-remove" title="Reject">' +
                          '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                          '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                          '</form><div>';
