$(document).ready(function () {
	...
	var table = $('#myTable').DataTable({
		...
		"bServerSide": true,
		"aoColumns": [
            { "sName": "GLCode", "sWidth": "30%" },
            { "sName": "TransCode", "sWidth": "30%" },
		...
		"fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
		    ...
		    var controller = '<div><form id="formApprove" action="' + baseUrl + 'ExtractSettingNonTransaction/Approve" method="post"  class="btn btn-info btn-sm fa fa-check" title="Approve">' +
                             '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                             '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                             '</form>';
		    controller += '<form id="formReject" action="' + baseUrl + 'ExtractSettingNonTransaction/Reject" method="post"  class="btn btn-info btn-sm fa fa-remove" title="Reject">' +
                          '<input id="glcode" name="glcode" type="hidden" value="' + glcode + '">' +
                          '<input id="trancode" name="trancode" type="hidden" value="' + trancode + '">' +
                          '</form><div>';
	...
	$('#myTable tbody').on("click", "#formApprove", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to approve this Extract Setting?", function (result) {
			if (result) {
				obj.submit();
			}
		});
	...
	$('#myTable tbody').on("click", "#formReject", function () {
		var obj = $(this);
		bootbox.confirm("Are you sure want to reject this Extract Setting?", function (result) {
			if (result) {
				obj.submit();
			}
		});
